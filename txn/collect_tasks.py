...
class CollectTasks(LegecloTransaction):
    """
    从能自由点击的主页开始领取任务实绩，最终再回到主页
    """
    TXN_NAME = "CollectTasks"
    ...
    def operate(self) -> None:
        # 点击任务
        self.ctl.touch_pos(pos.HOMEPAGE_TASK)
        time.sleep(5)
        # 切到实绩
        self.ctl.touch_pos(pos.TASK_ACHIEVEMENT)
        time.sleep(3)
        # 点击一键领取，一直点就行
        self.ctl.touch_pos_multiple(
            pos=pos.TASK_RECEIVE_ALL,
            interval=0.5,
            times=20,
        )  # 第一次领可能会卡
        # 点击主页
